...
BEGIN
  ...
  IF id_contr IS NULL OR id_staff IS NULL THEN
  ...
  IF id_doctype = 285 THEN
    FOR i IN 1 .. len
    LOOP
      SELECT "Lupa_A".count_goods(cast(good_names[i] AS integer)) INTO quant;
      IF quant - cast(quantities[i] AS double precision) < 0 THEN 
        RAISE EXCEPTION 'have no goods!';
      END IF;
    END LOOP;
 END IF;
